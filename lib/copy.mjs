 ...
 *
 * Hooks alone turned out not to be enough. They are one of seven slides, and
 * they are the slide config.copy.repeatHooks explicitly allows to repeat, so
 * feeding them back left the items - the actual body of the post - converging
 * on the same eight habits. `angles`, `worn` and `captions` are the rest of
 * the memory, compressed by lib/memory.mjs so that a queue of any depth costs
 * the prompt a fixed dozen lines.
 */
export async function draftPosts({
  ...
  const ask =
    buildAsk({ count, topic, recentHooks, repeatHooks, memory, angles, worn, captions }) +
  ...
  const stream = client.messages.stream({
    ...
    system: systemFor(itemCount, { memory }),
