...
itself. Two shapes are in use. Pick ONE and hold it for the whole carousel:

    SHORT (default)   a bare habit, 22-50 characters
        You only train when you feel like it
        You blame everyone else for your mistakes
        Not getting enough sleep
        Comparing yourself to others

    LABEL AND COST    the habit, a colon, then what it costs, 60-95 characters
        Not warming up properly: You risk getting an injury and it reduces your strength
        Comparing yourself to others: Destroys your confidence which leads to poor performance
...
not as sloppiness. Mixing the two SHAPES is not fine.
...
The renderer auto-fits every line to the slide, so your character count decides
the type size and nothing else. Measured on the real template: an item at 40
characters renders about 70% larger than the same item at 70, and roughly
double one at 80. Short items are not just punchier, they are physically bigger.
LABEL AND COST is the deliberate exception - it is meant to fill the frame.
...
- No terminal periods on any slide line. Mid-sentence commas are fine, and the
  colon in LABEL AND COST is fine.
...
/** Which item shape a line is written in. Used to catch a mixed set. */
function shapeOf(item) {
  return /:\s/.test(item) || item.length > 55 ? 'label and cost' : 'short';
}
...
export function lint(draft, itemCount) {
  ...
  // A set that mixes the two item shapes reads as a broken template rather
  // than as variety - the corpus never does it.
  const shapes = new Set(draft.items.filter(Boolean).map(shapeOf));
  if (shapes.size > 1) {
    warnings.push(`mixes item shapes (${[...shapes].join(' + ')}) - pick one for the whole carousel`);
  }
  const longForm = shapes.has('label and cost') && shapes.size === 1;
  ...
  for (const [i, item] of draft.items.entries()) {
    ...
    // The cap only applies to the short shape. LABEL AND COST is supposed to
    // fill the frame, and warning on it would fire on every line of a set that
    // is doing exactly what it was asked to do.
    if (!longForm && item.length > 70) {
      warnings.push(`item ${i + 1} is ${item.length} chars - renders roughly half the size of a 40-char line`);
    } else if (!longForm && item.length > 55) {
      warnings.push(`item ${i + 1} is ${item.length} chars - noticeably smaller on the slide than 40`);
    }
    if (longForm && item.length > 95) {
      warnings.push(`item ${i + 1} is ${item.length} chars - past 95 even the long shape stops fitting`);
    }
  }

  // The long shape needs more lines than render.py currently allows.
  if (longForm) {
    warnings.push('label-and-cost set: render.py caps at MAX_LINES = 4 and this shape wants 5-6 - check the render');
  }
