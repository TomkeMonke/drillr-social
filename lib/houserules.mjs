...
function renderExamples(examples) {
  ...
  // These used to be followed by "do NOT reuse their topics - sleep, junk
  // food, motivation and comparing yourself have all been done", which named
  // the four worn topics a fourth time in a prompt that had already named them
  // three times, and primed them while forbidding them. What is actually spent
  // is now computed from the queue and arrives with the request, so the corpus
  // can go back to teaching the one thing it is good for, which is shape.
  return `

THE TARGET VOICE
These are real posts from this account, transcribed off the slides. Match their
rhythm, their register and their length. They are here for SHAPE, not for
subject matter - their topics are spent, and what to write about instead comes
with the request below. Note that their item count may differ from what you
have been asked for; the count in your instructions wins.
...
Those five are shape references and nothing else - every one of them has
already been posted, so copy the form and leave the subject alone.
 ...
 *
 * `angles`, `worn` and `captions` are the account's memory, compressed by
 * memory.mjs. They are what stops every batch coming back as another five
 * lines about sleep and motivation, and between them they cost about fifteen
 * lines - less than the flat hook list they replaced. Read that module's
 * header for why the memory is a signal and not a transcript.
 *
 * Note the asymmetry, which is deliberate. The angles are stated firmly
 * because a positive instruction is what actually moves the output; the worn
 * words are stated softly because a hard ban does not make the model write
 * something better, it makes it write around the word, and "sleep" comes back
 * as "the hours you spend horizontal". Steer with the angles, nudge with the
 * worn list.
 */
export function buildAsk({
  ...
  if (recentHooks.length) {
    // Tolerate bare strings as well as { hook, uses }: the counted form is what
    // memory.mjs produces, but the older shape should not throw.
    const list = recentHooks
      .map((entry) => (typeof entry === 'string' ? { hook: entry, uses: 1 } : entry))
      .map(({ hook, uses }) => `- ${hook}${uses > 1 ? `  (used ${uses}x)` : ''}`)
      .join('\n');
    parts.push(
      repeatHooks
