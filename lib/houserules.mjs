...
function renderExamples(examples) {
  ...
  // Naming the spent topics here does prime them while forbidding them, which
  // is a real flaw. It is also the wording that produced the carousels actually
  // on the account, and the alternative - computing what is spent and steering
  // onto a fresh subject instead - was tried and made the copy worse. See the
  // note above buildAsk.
  return `
 ...
 *
 * ⚠️ DO NOT PUT THE SUBJECT IN HERE. A version of this steered every carousel
 * onto a drawn subject - "how much you use your weak foot", "what you do on
 * the bench" - and it was removed on 2026-09-08 after 25 real drafts, because
 * a subject that lives in the prompt does not reach a slide:
 *
 *     1. Avoiding it in every drill
 *     2. Passing around it instead of through it
 *     3. Never practising it outside training
 *
 * Nothing there says what "it" is, and slide 1 is the one that has to stop a
 * thumb. It also made all five items paraphrase the one subject, where the
 * real posts carry five DIFFERENT habits in a single carousel. It bought
 * variety between carousels by spending the variety inside them.
 *
 * The full write-up is in README, "The repetition problem". The code is in
 * git at 78d31ba if it is ever worth rebuilding, and rebuilding it needs a
 * subject that names itself on every slide, or one subject per BATCH.
 */
