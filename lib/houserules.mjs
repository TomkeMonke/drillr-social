...
function renderExamples(examples, memory) {
  ...
  // Two wordings of one paragraph, chosen by config.copy.memory.enabled.
  //
  // OFF (the default) is the wording that produced the carousels actually on
  // the account. It names the spent topics itself, which primes them while
  // forbidding them - a real flaw, but one the account survived, and the
  // alternative turned out worse. See the note above buildAsk.
  //
  // ON hands that job to the computed worn list instead.
  const spent = memory
    ? `They are here for SHAPE, not for
subject matter - their topics are spent, and what to write about instead comes
with the request below.`
    : `Do NOT reuse their topics - sleep,
junk food, motivation and comparing yourself have all been done.`;

  return `

THE TARGET VOICE
These are real posts from this account, transcribed off the slides. Match their
rhythm, their register and their length. ${spent} Note that
...
export function systemFor(itemCount, { memory = false } = {}) {
  return SYSTEM.replace(/\{\{ITEM_COUNT\}\}/g, String(itemCount)) + renderExamples(loadExamples(), memory);
}
 ...
 *
 * `memory` picks between two asks, and it is OFF by default.
 *
 * ⚠️ WHY IT IS OFF. The memory path (angles, worn words, counted hooks) does
 * fix the repetition it was built for - every batch stops being another five
 * lines about sleep - but drafting 25 real carousels through it showed what it
 * costs, and the cost is worse than the disease:
 *
 *   1. DANGLING PRONOUNS. The angle lives in the prompt and never reaches a
 *      slide, so the copy refers to it as if the reader can see it:
 *          1. Avoiding it in every drill        <- "it" is the weak foot
 *          2. Passing around it instead of through it
 *          3. Never practising it outside training
 *      Nothing on slides 1-3 says what "it" is. Same with "Treating them as a
 *      threat", where "them" is a new player in your position. The first slide
 *      is the one that has to stop a thumb, and it was meaningless.
 *
 *   2. FIVE WAYS OF SAYING ONE THING. Pinning a carousel to one angle makes
 *      all five items paraphrase it - five restatements of "you switch off on
 *      the bench" instead of five different habits. The real posts put sleep,
 *      junk food, motivation, stretching and comparison in ONE carousel. The
 *      angle bought variety between carousels by spending the variety inside
 *      them, and inside is where the reader actually notices.
 *
 * So the default ask is the one the account was really built on: previous
 * hooks as context, and nothing else. Turn config.copy.memory.enabled on only
 * with a fix for both of the above - an angle that names its own subject on
 * every slide, or one angle per batch rather than per carousel.
 */
export function buildAsk({
  ...
  if (memory && angles.length) {
    const list = angles.map((angle, i) => `  ${i + 1}. ${angle}`).join('\n');
    // The pool can run dry on a big batch - 50 angles against 60 carousels.
    // Saying so beats silently listing fewer angles than carousels and leaving
    // the model to work out whether it miscounted.
    const shortfall =
      angles.length < count
        ? `\n\nThat is ${angles.length} angle(s) for ${count} carousel(s) - the pool has no more ` +
          `fresh ones. Pick your own subject for the remaining ${count - angles.length}, staying ` +
          `well clear of the worn ground below.`
        : '';
    parts.push(
      `\nWHAT EACH ONE IS ABOUT\nOne angle per carousel, in this order. The angle decides what the ` +
        `items are about underneath the hook; the hook formulas themselves do not change.` +
        `\n\n${list}${shortfall}\n\n` +
        `A starting point, not a cage. If an angle will not give you a full set that ` +
        `sounds like this account, drop it and write the better set - copy that reads ` +
        `right beats copy that hit its brief.`
    );
  }

  if (memory && worn.length) {
    parts.push(
      `\nWORN OUT\nThese words are all over the back catalogue. They are NOT banned - where one ` +
        `is genuinely the right word, use it - but a set built mostly out of them is a set ` +
        `this account has already posted:\n  ${worn.join(', ')}`
    );
  }

  if (recentHooks.length) {
    // Two shapes reach this. With memory off the caller passes plain strings,
    // which is the list the account was actually drafted against; with it on,
    // { hook, uses } objects that also say which hooks are worn.
    const list = recentHooks
      .map((entry) => (typeof entry === 'string' ? { hook: entry, uses: 1 } : entry))
      .map(({ hook, uses }) => `- ${hook}${memory && uses > 1 ? `  (used ${uses}x)` : ''}`)
      .join('\n');
    ...
    if (!repeatHooks) {
      ...
      parts.push(
        `\nHooks already used, oldest first, with how many times each has run. Reusing a ` +
          `strong hook with a different set of items is fine - the account has done exactly ` +
          `that - but the ones with a count next to them are tired, and never write several ` +
          `variations on the same idea inside one batch:\n${list}`
      );
    } else {
      parts.push(
  ...
  if (memory && captions.length) {
    parts.push(`\nCaptions already used - write a different one:\n  ${captions.join(', ')}`);
  }
